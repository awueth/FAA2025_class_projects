...
/-- If `ω` is a primitive root of unity of even order `k`, then `ω ^ 2` is a primitive root of unity of order `k / 2`. -/
...
/-- If `ω` is a primitive root of unity of order `2 ^ (n + 1)`, then `ω ^ 2` is a primitive root of unity of order `2 ^ n`. -/
...
/-- Orthogonality of powers of a primitive root of unity -/
...
/-- Helper lemma to deal with coercions of `Fin n` to ℕ. -/
...
/-- If `ω` is a primitive root of unity of order `2 ^ (n + 1)`, then `ω ^ (2 ^ n) = -1` -/
