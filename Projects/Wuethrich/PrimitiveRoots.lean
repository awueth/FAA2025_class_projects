...
lemma of_pow_two {n : ℕ} (h : IsPrimitiveRoot ω (2 ^ (n + 1))) :
  ...
  have : 2 ^ n = 2 ^ n.succ / 2 := by grind
  rw [this]
  apply h.pow_two_of_even_order
  grind
...
lemma pow_two_fin_of_nat (h : IsPrimitiveRoot ω (2 ^ (n + 1))) (k : Fin (2 ^ (n + 1))) :
  ...
  conv_rhs =>
    rw [← Nat.mod_add_div k (2 ^ n)]
    ring_nf
    arg 2
    arg 2
    rw [mul_assoc, mul_comm _ 2, ← pow_succ', mul_comm]
  rw [pow_mul ω _ (↑k / 2 ^ n), h.pow_eq_one]
  ring
